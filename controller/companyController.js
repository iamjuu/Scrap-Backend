...
// ************************************************************
 agentCard :async (req, res) => {
  ...
  agentcarddata:async(req,res) =>{
    try{  const agentData = await CompanycardModel.find();
      console.log(agentData, 'data gotted');
      res.status(200).json({ message: true, Agent: agentData });

    }catch(err){
      console.log(err ,'err in the agentdata passing to frontend please check the company controller agent card data' );
